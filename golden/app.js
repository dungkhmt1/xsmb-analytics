...
// Hiển thị vị trí cầu, số ngày chạy và giới hạn đúng 2 gợi ý
function renderRank(el,rows){
 const top2 = (rows||[]).slice(0, 2);
 el.innerHTML = top2.map((x,i)=>`
 <div class="rank-row">
   <b>#${i+1}</b>
   <strong>${esc(x.number||x.x)}</strong>
   <span>Score: ${Number(x.score??x.s).toFixed(1)}</span>
   <small style="grid-column: 1 / 4; color: #b45309; font-size: 11px; margin-top: 2px;">
     Cầu: <b>${esc(x.bridgePosition||x.bridge||"Cầu tổng hợp")}</b> · Chạy liên tục: <b>${esc(x.runningDays??x.streak??0)} ngày</b>
   </small>
 </div>`).join("");
}
